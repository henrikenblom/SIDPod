...
int8_t DigiDetector::routeDigiSignal(int32_t *digi_out,
    ...
    if ((_used_digi_type != DigiNone) && _digi_enabled) {
        ...
        switch (_used_digi_type) {
            ...
            case DigiPWMTest:
                ...
                routeSignal((*digi_out), outf, outo, getSource() - 1);
...
int32_t
DigiDetector::getSample() {    // only for D418&PWM based samples (other techniques displayed directly in original voice)
...
void DigiDetector::recordSampleD418(uint8_t sample) {
    ...
    _current_digi_sample = ((int32_t) sample * 0x101 - 0x8000)
            >> 1; // D418-digis need to be less loud.. (but not voice specific digis)
    _current_digi_src = 0;
    _digi_count++;
        ...
        switch (value) {
            ...
            case 0x41:    // PULSE/GATE
                if (((_pulse_detect_state[voice] == PulseConfirm) || (_pulse_detect_state[voice] == PulseConfirm2))
                    ...
                    setMuted(voice, true);        // test-case: Wonderland_XII-Digi_part_1
...
uint8_t DigiDetector::setSwallowMode(uint8_t voice, uint8_t m) {
    ...
    setMuted(voice, true);    // avoid wheezing base signals created by regular voice output
...
uint8_t DigiDetector::handleSwallowDigi(uint8_t voice, uint8_t reg,
    ...
    if (reg == 4) {
        if ((getWave(voice) & 0x8) && !(value & 0x8) && (value & 0x40)
            && ((getAD(voice) == 0) && (getSR(voice) == 0xf0))) {
...
int32_t DigiDetector::genPsidSample(int32_t sample_in) {
    ...
    if ((_sample_position < _sample_end) && (_sample_position >= _sample_start)) {
        ...
        if (_frac_pos > (int32_t) SAMPLE_RATE) {
            _frac_pos %= SAMPLE_RATE;
...
uint8_t DigiDetector::detectSample(uint16_t addr, uint8_t value) {
    ...
    if (!_is_compatible) {
        ...
        if ((addr == (_base_addr + 0x18)) && assertSameSource(0)) {
            recordSampleD418(isMahoneyDigi() ? _mahoneySample[value]
                                             : getD418Sample(value));
        }
    }
    return 0;
