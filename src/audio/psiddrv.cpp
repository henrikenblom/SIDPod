...
uint8_t psiddrv::iomap(uint_least16_t addr) const
{
    ...
    if (m_tuneInfo->compatibility == TuneInfo::COMPATIBILITY_R64
        || m_tuneInfo->compatibility == TuneInfo::COMPATIBILITY_BASIC
        || addr == 0)
    {
...
{
    const int startlp = m_tuneInfo->load >> 8;
    const int endlp   = (m_tuneInfo->load + (m_tuneInfo->c64dataLen - 1)) >> 8;

    uint_least8_t relocStartPage = m_tuneInfo->relocStartPage;
    uint_least8_t relocPages = m_tuneInfo->relocPages;

    if (m_tuneInfo->compatibility == TuneInfo::COMPATIBILITY_BASIC)
    {
    ...
    reloc_size   = static_cast<int>(psid_driver.size());
...
void psiddrv::install(sidmemory& mem, uint8_t video) const
{
    ...
    else
    {
        ...
        mem.fillRam(0x0314, &reloc_driver[2], m_tuneInfo->compatibility == TuneInfo::COMPATIBILITY_R64 ? 2 : 6);
    ...
    mem.writeMemByte(pos, *m_tuneInfo->songSpeed == TuneInfo::SPEED_VBI ? 0 : 1);
    pos++;

    // Set init address
    mem.writeMemWord(pos, m_tuneInfo->compatibility == TuneInfo::COMPATIBILITY_BASIC ?
                     0xbf55 : m_tuneInfo->init);
    pos += 2;

    // Set play address
    mem.writeMemWord(pos, m_tuneInfo->play);
    pos += 2;

    // Set init address io bank value
    mem.writeMemByte(pos, iomap(m_tuneInfo->init));
    pos++;

    // Set play address io bank value
    mem.writeMemByte(pos, iomap(m_tuneInfo->play));
    pos++;
    ...
    mem.writeMemByte(pos, m_tuneInfo->compatibility >= TuneInfo::COMPATIBILITY_R64 ? 0 : 1 << MOS6510::SR_INTERRUPT);
}
